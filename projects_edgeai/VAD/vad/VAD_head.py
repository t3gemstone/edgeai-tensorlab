...
class VADHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        self.map_bbox_coder = TASK_UTILS.build(map_bbox_coder)
        ...
        super(AnchorFreeHead, self).__init__(init_cfg=None)
        ...
        if train_cfg is not None:
            assert 'map_assigner' in train_cfg, 'map assigner should be provided '\
                'when train_cfg is set.'
    ...
    def forward(self,
        ...
        # Do the same for inter_inferences
        #if torch.onnx.is_in_onnx_export():
        #    hs=hs.reshape(map_hs.shape[0], -1, 1, self.embed_dims)
        #    inter_references = inter_references.reshape(map_hs.shape[0], 1, -1, 3)
        ...
        if torch.onnx.is_in_onnx_export():
            ...
            outputs_coords, outputs_coords_bev = \
                self.run_reg_branch(batch_hs, init_reference, inter_references)
        else:
            ...
            for lvl in range(hs.shape[0]):
                ...
                outputs_coord = tmp
                outputs_classes.append(outputs_class) # outputs_class: [1, 300, 10]
                outputs_coords.append(outputs_coord)  # outputs_coord: [1, 300, 10]
        ...
        if self.motion_decoder is not None:
            ...
            # unflatten throws error while exporting
            ...
            motion_hs = motion_hs.permute(1, 0, 2).reshape(
                batch_size, num_agent, self.fut_mode, motion_hs.shape[-1]
            )
            ca_motion_query = ca_motion_query.squeeze(0).reshape(
                batch_size, num_agent, self.fut_mode, ca_motion_query.shape[-1]
            )
    ...
    def run_reg_branch(self, batch_hs, init_reference, inter_references):
        ...
        out  = torch.add(out, reg_linear3_bias) # out: [3x300x10]
        ...
        outputs_coords_bev = temp[..., 0:2].unsqueeze(1).clone().detach()
        ...
        out = list(out.unsqueeze(1))  # To revisit to keep Tensor
        outputs_coords_bev = list(outputs_coords_bev) # To revisit to keep Tensor
        return out, outputs_coords_bev
    ...
    def _map_get_target_single(self,
        ...
        sampling_result = self.map_sampler.sample(assign_result, InstanceData(priors=bbox_pred),
                                                  InstanceData(bboxes_3d=gt_bboxes))
        pos_inds = sampling_result.pos_inds
        neg_inds = sampling_result.neg_inds
        # label targets
