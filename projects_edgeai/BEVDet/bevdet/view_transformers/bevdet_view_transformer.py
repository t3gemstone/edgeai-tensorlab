...
class LSSViewTransformer(BaseModule):
    ...
    #                   bda):    
    def get_lidar_coor(self, sensor2ego, cam2img, lidar2cam, ege2global, post_rts, bda):
        ...
        post_trans = torch.zeros([B, N, 3]).to(sensor2ego)
        post_rots = torch.zeros([B, N, 3, 3]).to(sensor2ego)
        post_trans[:, :, :2] = post_rts[:, :, :2, 2]
        post_rots[:, :, :2, :2] = post_rts[:, :, :2, :2]
        post_rots[:, :, 2, 2] = 1

        points = self.frustum.to(sensor2ego) - post_trans.view(B, N, 1, 1, 1, 3)
        points = torch.inverse(post_rots).view(B, N, 1, 1, 1, 3, 3)\
            .matmul(points.unsqueeze(-1))
        #points = self.frustum.to(sensor2ego).view(B, N, 59, 16, 44, 3, 1)

        # cam_to_ego
        points = torch.cat(
            (points[..., :2, :] * points[..., 2:3, :], points[..., 2:3, :]), 5)
        combine = sensor2ego[:,:,:3,:3].matmul(torch.inverse(cam2img[:, :, :3, :3]))
    ...
    def view_transform_core(self, img, sensor2ego, cam2img, lidar2cam, ego2global, post_rts, bda,
                            depth, tran_feat):
        B, N, C, H, W = img.shape
    ...
    def forward(self, img, sensor2ego, cam2img, lidar2cam, ego2global, post_rts, bda, depth_from_lidar=None):
        ...
        return self.view_transform(img, sensor2ego, cam2img, lidar2cam, ego2global, post_rts, bda,
                                   depth, tran_feat)
