...
class StreamPETRHead(AnchorFreeHead):
    ...
    def init_memory(self, x):
        B = x.size(0)
        memory_embedding = x.new_zeros(B, self.memory_len, self.embed_dims)
        memory_reference_point = x.new_zeros(B, self.memory_len, 3)
        memory_timestamp = x.new_zeros(B, self.memory_len, 1)
        memory_egopose = x.new_zeros(B, self.memory_len, 4, 4)
        memory_velo = x.new_zeros(B, self.memory_len, 2)

        return memory_embedding, memory_reference_point, memory_timestamp, \
            memory_egopose, memory_velo

    def pre_update_memory(self, img_feats, img_metas,
    ...
    def position_embeding(self, img_feats, memory_centers, topk_indexes, img_metas):
        eps = 1e-5
        ...
        intrinsics = img_feats.new_tensor(intrinsics)  # (B, N, 4, 4)
        img2lidars = np.asarray(img2lidars)
        img2lidars = img_feats.new_tensor(img2lidars)  # (B, N, 4, 4)
    ...
    def forward(self, memory_center, img_feats, img_metas,
        ...
        self.pre_update_memory(img_feats, img_metas,
        ...
        pos_embed, cone = self.position_embeding(img_feats, memory_center, topk_indexes, img_metas)
